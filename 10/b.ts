...
function setNextNode(node: ListNode, grid: string[][], length = 1) {
    ...
    if (nextY !== null && nextX !== null) {
        ...
        node.next = {
            value: grid[nextY][nextX] as ListNode['value'],
            next: null,
            x: nextX,
            y: nextY,
            prev: node,
...
setNextNode(startNode!.paths[0], grid);

const cleanGrid = grid.map((row) => row.map(() => '.'));
cleanGrid[startNode!.y][startNode!.x] = 'S';
const nodeMap = new Map<string, ListNode>();
let currNode: ListNode | null = startNode!.paths[0];
while (currNode !== null) {
    const { x, y } = currNode;
    cleanGrid[y][x] = currNode.value;
    nodeMap.set(`${x},${y}`, currNode);
    currNode = currNode.next;
}

// Set a pipe dir for starting point
const first = startNode?.paths[0];
const last = startNode?.paths[1];
if (first && last) {
    if (first.x === last.x) {
        cleanGrid[startNode.y][startNode.x] = '|';
    } else if (first.y === last.y) {
        cleanGrid[startNode.y][startNode.x] = '-';
    } else if (first.x < last.x && first.y < last.y) {
        cleanGrid[startNode.y][startNode.x] = '7';
    } else if (first.x < last.x && first.y > last.y) {
        cleanGrid[startNode.y][startNode.x] = 'J';
    } else if (first.x > last.x && first.y < last.y) {
        cleanGrid[startNode.y][startNode.x] = 'F';
    } else if (first.x > last.x && first.y > last.y) {
        cleanGrid[startNode.y][startNode.x] = 'L';
    }
}

function getNumCrosses(row: string[]) {
    let numCrosses = 0;
    // Working our way across the row
    ...
    for (let i = 0; i < row.length; i++) {
        ...
        if (curr === '|') {
            ...
            numCrosses++;
            previousTurn = null;
        } else if (curr === 'F' || curr === 'L') {
            // ---> F-? --->
            // ---> L-? --->
            previousTurn = curr;
        } else if (
            (previousTurn === 'F' && curr === 'J') ||
            (previousTurn === 'L' && curr === '7')
        ) {
            // ---> F-J --->
            // ---> L-7 --->
            numCrosses++;
            ...
            (previousTurn === 'F' && curr === '7') ||
            (previousTurn === 'L' && curr === 'J')
        ) {
            // ---> F-7 --->
            // ---> L-J --->
            previousTurn = null;
...
let numCellsInside = 0;
cleanGrid.forEach((row, y) => {
    row.forEach((cell, x) => {
        if (cell === '.') {
            const leftOfCell = cleanGrid[y].slice(0, x);
            const rightOfCell = cleanGrid[y].slice(x + 1);

            const numCrossesLeft = getNumCrosses(leftOfCell);
            const numCrossesRight = getNumCrosses(rightOfCell);

            if (numCrossesLeft % 2 === 1 && numCrossesRight % 2 === 1) {
                cleanGrid[y][x] = '1';
                numCellsInside++;
            } else {
                cleanGrid[y][x] = '0';
...
console.log('Num cells inside', numCellsInside);
