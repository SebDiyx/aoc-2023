 ...
 */
const CARD_RANKING = 'J23456789TQKA';
const handTypes = [
    'five-of-a-kind',
    'four-of-a-kind',
    'full-house',
...
const hands: Hand[] = input.split('\n').map((line) => {
    ...
    const [maxKey] = [...cardsMap.entries()].reduce(
        (max, [key, val]) => {
            if (max === null) return [key, val];
            if (val > max[1]) return [key, val];
            return max;
        },
        ['n/a', 0] as [string, number],
    );
    cardsMap.set(maxKey, cardsMap.get(maxKey) + numJokers);
